...
sealed trait FlutterbyCallback
object FlutterbyCallback {

  final case class ConfigAwareCallback(f: FlutterbyConfig => Callback) extends FlutterbyCallback

  trait Callback extends FlutterbyCallback {
  ...
  def toFlyway(c: FlutterbyCallback, flutterbyConfig: FlutterbyConfig): FlywayCallback = c match {
    case callback: Callback      => Callback.toFlyway(callback)
    case ConfigAwareCallback(fn) => Callback.toFlyway(fn(flutterbyConfig))
  }
...
final case class Callbacks(callbacks: Vector[FlutterbyCallback])
